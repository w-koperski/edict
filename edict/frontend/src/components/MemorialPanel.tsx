...
export default function MemorialPanel() {
  ...
  const exportMemorial = (t: Task) => {
    const fl = t.flow_log || [];
    let md = `# 📜 Memorial · ${t.title}\n\n`;
    md += `- **Task ID**: ${t.id}\n`;
    md += `- **Status**: ${t.state}\n`;
    md += `- **Responsible Dept**: ${t.org}\n`;
    if (fl.length) {
      ...
      md += `- **Start Time**: ${startAt}\n`;
      md += `- **Completion Time**: ${endAt}\n`;
    }
    md += `
## Flow Log\n\n`;
    for (const f of fl) {
    ...
    if (t.output && t.output !== '-') md += `## Output\n\n\`${t.output}\`\n`;
    navigator.clipboard.writeText(md).then(
      () => toast('✅ Memorial copied as Markdown', 'ok'),
  ...
  return (
    <div>
      ...
      <div className="mem-list">
        {!mems.length ? (
          <div className="mem-empty">No memorials yet — auto-generated when tasks complete</div>
        ) : (
          mems.map((t) => {
            const fl = t.flow_log || [];
            const depts = [...new Set(fl.map((f) => f.from).concat(fl.map((f) => f.to)).filter((x) => x && x !== 'Emperor'))];
              ...
              <div className="mem-card" key={t.id} onClick={() => setDetailTask(t)}>
                ...
                <div className="mem-info">
                  ...
                  <div className="mem-sub">
                    {t.id} · {t.org || ''} · Flow: {fl.length} steps
                  </div>
...
function MemorialDetailModal({
  ...
  for (const f of fl) {
    if (f.from === 'Emperor') originLog.push(f);
    else if (f.to === 'Zhongshu' || f.from === 'Zhongshu') planLog.push(f);
    else if (f.to === 'Menxia' || f.from === 'Menxia') reviewLog.push(f);
    else if (f.remark && (f.remark.includes('Complete') || f.remark.includes('Report Back'))) resultLog.push(f);
    else execLog.push(f);
  }
    ...
    return (
      <div style={{ marginBottom: 18 }}>
        ...
        <div className="md-timeline">
          {items.map((f, i) => {
            const dotCls = f.remark?.includes('✅') ? 'green' : f.remark?.includes('Reject') ? 'red' : '';
            return (
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 18, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--muted)' }}>Flow: {fl.length} steps</span>
            {depts.map((d) => (
              <span className="mem-tag" key={d}>{d}</span>
            ))}
          </div>
          ...
          {renderPhase('Imperial Edict', '👑', originLog)}
          {renderPhase('Zhongshu Plan', '📋', planLog)}
          {renderPhase('Menxia Review', '🔍', reviewLog)}
          {renderPhase('Six Ministries Execution', '⚔️', execLog)}
          {renderPhase('Summary Report', '📨', resultLog)}
