...
export default function MonitorPanel() {
  ...
  const handleWakeAll = async () => {
    ...
    toast(`Waking up ${toWake.length} agents...`);
    for (const a of toWake) {
      try { await api.agentWake(a.id); } catch { /* ignore */ }
    }
    toast(`${toWake.length} wake commands sent, refreshing status in 30s`);
    setTimeout(() => loadAgentsStatus(), 30000);
  };
      ...
      {asData && asData.ok && (
        <div className="as-panel">
          <div className="as-header">
            <span className="as-title">🔌 Agent Online Status</span>
              ...
              return (
                <div key={a.id} className="as-card" title={`${a.role} · ${a.statusLabel}`}>
                  ...
                  {a.lastActive ? (
                    ...
                    <div style={{ fontSize: 10, color: 'var(--muted)' }}>No activity record</div>
                  )}
                  {canWake && (
          ...
          <div className="as-summary">
            ...
            <span><span className="as-dot idle" style={{ position: 'static', width: 8, height: 8 }} /> {idle} Standby</span>
                ...
                {myTasks.length > 0 ? (
                  myTasks.map((t) => (
                    <div key={t.id} className="dc-task" onClick={() => setModalTaskId(t.id)}>
                      ...
                      <div className="dc-task-title">{t.title || '(no title)'}</div>
                      {t.now && t.now !== '-' && (
